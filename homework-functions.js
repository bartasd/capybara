...
const asd = new Array(100)
  .fill(0)
  .map((el) => Math.floor(Math.random() * 44 + 33))
  .sort((a, b) => divs(a) - divs(b));
console.log(asd);
...
const asd2 = new Array(100)
  .fill(0)
  .map((el) => Math.floor(Math.random() * 444 + 333))
  .filter((a) => divs(a) === 0);
console.log(asd2);
...
for (let i = 0; i < repeats; i++) {
  if (i === 0) {
    const temp = new Array(Math.floor(Math.random() * 10 + 10))
      .fill(1)
      .map((el) => Math.floor(Math.random() * 10));
    temp.push(0);
    asd3 = [...temp];
  } else {
    const temp = new Array(Math.floor(Math.random() * 10 + 10))
...
let asd4 = [...asd3];
while (!asd4.every((el) => typeof el === "number")) asd4 = asd4.flat();
...
const asd5 = new Array(3)
  .fill(0)
  .map((el) => (el = Math.floor(Math.random() * 32 + 1)));
generate(asd5);
console.log("asd5: ", asd5);

/*9. Sugeneruokite masyvą iš 10 elementų, kurie yra masyvai iš 10 elementų, 
...
const bigis = new Array(10)
  .fill(0)
  .map((mazis) =>
    new Array(10).fill(0).map((element) => Math.floor(Math.random() * 99 + 1))
  );
console.log(bigis);

const bigiubigis = bigis.flat();

let bigger70 = false;
let primesofbiggies = null;
let sumPrimes = null;
let average = null;

while (bigger70 === false) {
  primesofbiggies = bigiubigis.filter((el) => isPrime(el));
  sumPrimes = primesofbiggies.reduce((a, b) => a + b, 0);
  average = sumPrimes / primesofbiggies.length;
  if (average > 70) {
    bigger70 = true;
  } else {
    bigiubigis.sort((a, b) => a - b);
    bigiubigis[0] += 3;
  }
}
console.log("primesAverage: ", average);
